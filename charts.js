...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var arrays = resultArray[0];
    // Deliverable 3: 2. Create a variable that holds the first sample in the metadata array.
    var metaArray = metadata[0];
    // Deliverable 1: 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    var otu_ids = arrays.otu_ids;
    var otu_labels = arrays.otu_labels;
    var sample_values = arrays.sample_values;
    // Deliverable 3: 3. Create a variable that holds the washing frequency.
    var wash_freq = parseFloat(metaArray.wfreq);
    ...
    var gaugeData = [
      {
        value: wash_freq,
        title: { text: "Belly Button Washing Frequency (Scrubs per Week)", font: { size: 18, color: "black" }},
        type: "indicator",
        mode: "gauge+number",
        gauge: {
          axis: { range: [null, 10] },
